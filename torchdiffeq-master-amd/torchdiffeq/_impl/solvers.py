...
class AMDAdaptiveStepsizeODESolver(object):
    __metaclass__ = abc.ABCMeta

    def __init__(self, func, floss, y0, atol, rtol, **unused_kwargs):
    ...
    def integrate(self, t):
        ...
        #         solution.append(y)
        #     # else: 
        #     #     solution.append(y) #just add the solution to the array if convergent
        ...
        self.before_integrate(t)
        y=None
        while not self.convergent and t[-1] > self.amd_state.t1:
            self.amd_state = self._adaptive_amd_step(self.amd_state, t[-1] )            
            if self.accept_step:
                tlist.append(self.amd_state.t1)
                solution.append(self.amd_state.y1)
        print('>>> Rejected atempts:', self.reject_counter)
        return tuple(map(torch.stack, tuple(zip(*solution)))), torch.tensor(tlist)
