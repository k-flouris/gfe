...
def _compute_error_ratio_amd(error_estimate, error_tol=None, rtol=None, atol=None, y0=None, y1=None):
    ...
    # print( 'error_tol_=', error_tol) #, #'factor=',factor, 'df=', 1 / ifactor)
    # print( 'error_estimate=', error_estimate) #, #'factor=',factor, 'df=', 1 / ifactor)
    error_ratio = tuple(error_estimate_ / error_tol_ for error_estimate_, error_tol_ in zip(error_estimate, error_tol))
    mean_sq_error_ratio = tuple(torch.mean(error_ratio_ * error_ratio_ ) for error_ratio_ in error_ratio)
    # print( 'error_tol=', error_tol, 'error_ratio=', error_ratio, 'mean_sq_error_ratio=', mean_sq_error_ratio) #, #'factor=',factor, 'df=', 1 / ifactor)
    # import sys
    # sys.exit(0)
...
def _optimal_step_size(last_step, mean_error_ratio, safety=0.9, ifactor=10.0, dfactor=0.2, order=5):
    ...
    # print( 'ls=', last_step, 'factor=',factor, 'df=', 1 / ifactor)
    return last_step / factor

def _optimal_step_size_amd(last_step, mean_error_ratio, safety=0.9, ifactor=10.0, dfactor=0.2, order=5):
    """Calculate the optimal size for the next step."""
    mean_error_ratio = max(mean_error_ratio)  # Compute step size based on highest ratio.
    if mean_error_ratio == 0:
        return last_step * ifactor
    if mean_error_ratio < 1:
        dfactor = _convert_to_tensor(0.8, dtype=torch.float64, device=mean_error_ratio.device)
    error_ratio = torch.sqrt(mean_error_ratio).to(last_step)
    exponent = torch.tensor(1 / order).to(last_step)
    factor = torch.max(1 / ifactor, torch.min(error_ratio**exponent / safety, 1 / dfactor))
    # print( 'ls=', last_step, 'factor=',factor, 'df=', 1 / ifactor)
...
def _check_inputs_amd(func, floss, y0, t):
    # check floss in same way or change?
    tensor_input = False
    if torch.is_tensor(y0):
        tensor_input = True
        y0 = (y0,)
        _base_nontuple_func_ = func
        _base_nontuple_floss_ = floss
        func = lambda t, y: (_base_nontuple_func_(t, y[0]),)
        floss= lambda y: (_base_nontuple_floss_(y[0]),)
    assert isinstance(y0, tuple), 'y0 must be either a torch.Tensor or a tuple'
    for y0_ in y0:
        assert torch.is_tensor(y0_), 'each element must be a torch.Tensor but received {}'.format(type(y0_))
    ...
    return tensor_input, func, floss, y0, t
